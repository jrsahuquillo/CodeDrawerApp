...
$(document).on('turbolinks:load', function () {
  ...
  function filterNotifications(notification) {
    ...
    if (notification.notifiable["type"] == "Friendship") {
      $('#notifications .dropdown-menu').append(common_html + " you</small></div>");
    } else if (notification.notifiable["type"] == "Drawer") {
      if (notification.action == "added you as collaborator of") {
        $('#notifications .dropdown-menu').append(common_html + "<a href=" + notification.notifiable['path'] + " class='notification-badge'> " + notification.notifiable['title'] + "</a></small></div>");
      } else {
        $('#notifications .dropdown-menu').append(common_html + " <strong>" + notification.notifiable['title'] + "</strong></small></div>");
      }
    } else if (notification.notifiable["type"] == "FavoriteCodetool") {
      $('#notifications .dropdown-menu').append(common_html + "<strong><a href=" + notification.notifiable['path'] + " class='notification-badge'> " + notification.notifiable['title'] + "</a></strong></small></div>");
    } else if (notification.notifiable["type"] == "Codetool") {
      $('#notifications .dropdown-menu').append(common_html + "<strong><a href=" + notification.notifiable['path'] + " class='notification-badge'> " + notification.notifiable['title'] + "</a></strong></small></div>");
    } else if (notification.notifiable["type"] == "User") {
      $('#notifications .dropdown-menu').append("<div class='dropdown-item' id='notification_" + notification.id + "'><small> Welcome to notifications! </small></div>");
    }
  }
